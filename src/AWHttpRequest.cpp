...
#include "AWHttpRequest.h"

namespace OpenAPI {

AWHttpRequestInput::AWHttpRequestInput() {
    initialize();
}

AWHttpRequestInput::AWHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void AWHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void AWHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void AWHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    AWHttpFileElement file;
...
AWHttpRequestWorker::AWHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
AWHttpRequestWorker::~AWHttpRequestWorker() {
...
QMap<QString, QString> AWHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

AWHttpFileElement AWHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return AWHttpFileElement();
}

QByteArray *AWHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void AWHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void AWHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void AWHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void AWHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  AWHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString AWHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void AWHttpRequestWorker::execute(AWHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<AWHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (AWHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*AWHttpRequestWorker::sslDefaultConfiguration);
    }
...
void AWHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
    ...
    if (reply->rawHeaderPairs().size() > 0) {
...
void AWHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void AWHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        ...
        if ((contentDisposition.size() > 0) && (contentDisposition.first() == QString("attachment"))) {
        ...
        if ((contentType.size() > 0) && (contentType.first() == QString("multipart/form-data"))) {
...
QByteArray AWHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray AWHttpRequestWorker::compress(const QByteArray& input, int level, AWCompressionType compressType) {
...
QSslConfiguration *AWHttpRequestWorker::sslDefaultConfiguration;
